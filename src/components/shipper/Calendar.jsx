...
function parseDateFlexible(value) {
  ...
  // Firestore-ish timestamp objects
  ...
  // ISO / RFC / other date-ish strings
...
export default function Calendar() {
  ...
  const effectiveProvider = useMemo(() => {
    try {
      ...
      if (fromCb) {
        localStorage.setItem('fp_shipper_calendar_provider', fromCb);
        return fromCb;
      }
      return String(localStorage.getItem('fp_shipper_calendar_provider') || '').trim();
    } catch {
      return '';
    }
  }, [location.search]);
  ...
  const uniqueCarriersFromLoads = useMemo(() => {
    const byId = new Map();
    for (const l of Array.isArray(loads) ? loads : []) {
      const uid = String(l?.assigned_carrier || l?.assigned_carrier_id || l?.carrier_id || '').trim();
      if (!uid) continue;
      const name = String(l?.assigned_carrier_name || l?.carrier_name || 'Carrier').trim();
  ...
  const fetchMonthData = async () => {
    ...
    try {
      ...
      if (isAuthFailure) {
        ...
        // Partial failures still matter; show a warning so it doesn't look like the calendar is "not responding".
  ...
  const derivedEvents = useMemo(() => {
    ...
    for (const e of Array.isArray(internalEvents) ? internalEvents : []) {
      ...
      items.push({
        ...
        assigned_carrier_uids: Array.isArray(e?.assigned_carrier_uids) ? e.assigned_carrier_uids : [],
      });
    }
  ...
  const startCalendarConnect = async (provider) => {
    ...
    try {
      const token = await currentUser.getIdToken();
      const returnTo = `/shipper-dashboard?nav=calendar`;
      ...
      localStorage.setItem('fp_shipper_calendar_provider', prov);
  ...
  const syncExternal = async (provider) => {
    ...
    try {
      ...
      // Sync a bounded set of near-term events (month + next 60 days) so external calendars stay useful.
      const now = new Date();
  ...
  const createInternalEvent = async () => {
    ...
    // Store end as an exclusive date (calendar providers expect this for all-day events).
    ...
    try {
      ...
      // Assign to carriers if requested.
      const shouldAssign = Boolean(form.assign_to_carriers) && Array.isArray(form.carrier_uids) && form.carrier_uids.length > 0;
      if (shouldAssign && created?.id) {
        await assignEventToCarriers(String(created.id), form.carrier_uids);
      }

      // Sync to external if connected.
  ...
  const assignEventToCarriers = async (eventId, carrierUids) => {
    ...
    try {
      ...
      await fetch(`${API_URL}/calendar/internal/events/${encodeURIComponent(eventId)}/assign`, {
        ...
        body: JSON.stringify({ carrier_uids: Array.isArray(carrierUids) ? carrierUids : [], sync_external: true }),
      });
    } catch {
      // ignore
    }
  ...
  return (
    <div className="calendar-container">
      ...
      {dayDetails ? (
        <div className="ic-modal-backdrop" onClick={() => setDayDetails(null)} role="dialog" aria-modal="true">
          <div className="ic-modal" onClick={(e) => e.stopPropagation()} style={{ width: 620, maxWidth: '100%' }}>
            <div className="ic-modal-header">
              <h3>Events — {dayDetails.ymd}</h3>
              <button className="ic-close" onClick={() => setDayDetails(null)} aria-label="Close">×</button>
            </div>
            <div className="ic-modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: '70vh', overflowY: 'auto' }}>
            ...
            <div className="ic-modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div className="card" style={{ padding: 12 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <input
                    type="checkbox"
                    checked={Boolean(form.assign_to_carriers)}
                    onChange={(e) => setForm((p) => ({ ...p, assign_to_carriers: e.target.checked, carrier_uids: [] }))}
                    disabled={uniqueCarriersFromLoads.length === 0}
                  />
                  <span>Add event to carriers calendars</span>
                </label>
                {uniqueCarriersFromLoads.length === 0 ? (
                  <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>No assigned carriers found on your loads yet.</div>
                ) : null}

                {form.assign_to_carriers ? (
                  <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <div className="muted" style={{ fontSize: 12 }}>Select carriers</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                      {uniqueCarriersFromLoads.map((c) => {
                        const checked = form.carrier_uids.includes(c.uid);
                        return (
                          <label key={c.uid} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={(e) => {
                                ...
                                setForm((p) => {
                                  const cur = Array.isArray(p.carrier_uids) ? p.carrier_uids : [];
                                  const set = new Set(cur);
                                  if (next) set.add(c.uid);
                                  else set.delete(c.uid);
                                  return { ...p, carrier_uids: Array.from(set) };
                                });
                              }}
                            />
              ...
              <div className="muted" style={{ fontSize: 12 }}>
                Reminders and carrier notifications are sent in-app, and external sync uses your connected provider.
              </div>
            </div>
          </div>
        </div>
